...
function GetParentId($id=0)
{
    ...
    $r = $dosql->GetOne("SELECT * FROM `#@__infoclass` WHERE `id`=$id");

    if(isset($r['parentstr'])){

        if(GetTopID($r['parentstr'])==0){
            return $r['id'];
        }
        else
        {
            return GetTopID($r['parentstr']);
        }
    }
    return 0;
}
...
function GetParentClassName($id=0)
{
    global $dosql;
    ...
    if(isset($r['parentstr'])) {
        ...
        if (GetTopID($r['parentstr']) == 0) {
            ...
            $pid = GetTopID($r['parentstr']);
            $classname = $dosql->GetOne("SELECT classname FROM `#@__infoclass` WHERE id = $pid");
            return $classname;
        }
    }
    return 0;
 ...
 */
function footerLogoPic($cid=0,$title='')
{
    global $dosql;

    $r = $dosql->GetOne("SELECT `picurl` FROM `#@__admanage` WHERE `classid`=$cid and title='footer_logo' " );
...
/*
 * 函数说明：单页标题 (必须自定义name字段)
 *
 * @access  public
 * @param   $classid  int     类别ID
 * @return            string  单页标题 (必须自定义title字段)
 */
function InfoClassName($cid=0)
{
    global $dosql;

    $r = $dosql->GetOne("SELECT `name` FROM `#@__info` WHERE `classid`=$cid");
    if(isset($r) && is_array($r))
    {
        return $r['name'];
    }
